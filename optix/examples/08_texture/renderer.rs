use cu::{
    allocator::{DeviceFrameAllocator, Layout, DeviceAllocRef, DeviceAllocImpl},
    Allocation,
    DevicePtr,
};
...
impl Renderer {
    pub fn new(
        ...
        let color_buffer = optix::TypedBuffer::<V4f32, _>::uninitialized_in(
        ...
        drop(triangle_inputs); //< make borrowck happy
        Ok(Renderer {
            ctx,
            stream,
            launch_params,
    ...
    pub fn render(&mut self) -> Result<()> {
        ...
        // stop-the-world
        cu::Context::synchronize().context("Launch synchronize")?;
