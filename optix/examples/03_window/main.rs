...
fn main() {
    ...
    let mut sample =
        SampleRenderer::new(IVec2::new(width as i32, height as i32)).unwrap();
    ...
    let mut image_data =
        vec![vec4(0.0, 0.0, 0.0, 0.0); (width * height) as usize];
    ...
    while !window.should_close() {
        ...
        if (w != width || h != height) {
            fsq.resize(w, h);
            sample.resize(IVec2::new(w as i32, h as i32));
            width = w;
            height = h;
            image_data
                .resize((width * height) as usize, vec4(0.0, 0.0, 0.0, 0.0));
        }
